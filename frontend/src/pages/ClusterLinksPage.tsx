...
import {
  ...
  LayoutList, LayoutGrid, GripVertical, Table2, ChevronLeft, ChevronRight, SlidersHorizontal, Info,
} from 'lucide-react';
import {
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-xs text-muted-foreground">
            표를 아래/옆으로 늘려 더 많은 링크를 확인할 수 있습니다.
          </p>
          <div className="relative">
            <button
              onClick={() => setShowTableSettings((prev) => !prev)}
              className="px-3 py-1.5 text-xs rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 inline-flex items-center gap-1.5 shadow-sm"
            >
              <SlidersHorizontal className="w-3.5 h-3.5" />
              표 스타일
            </button>
            {showTableSettings && (
              <div className="absolute right-0 top-9 z-20 w-[440px] max-w-[88vw] bg-white border border-slate-200 rounded-xl p-4 shadow-xl space-y-3">
        ...
        <div className="border border-slate-200 rounded-xl bg-slate-50/80 p-2 shadow-sm resize-y overflow-auto" style={{ height: tableHeight }}>
          ...
          {pagedGroups.map((g, idx) => (
            ...
            >
              ...
              <span className="font-semibold text-xs truncate inline-flex items-center gap-1" style={{ color: appearance.textColor }}>
                {g.clusterName}
                <span className="group relative inline-flex">
                  <Info className="w-3 h-3 text-slate-400" />
                  <span className="absolute left-1/2 -translate-x-1/2 top-4 hidden group-hover:block whitespace-nowrap bg-slate-900 text-white text-[10px] px-2 py-1 rounded-md z-10 max-w-[240px] truncate">
                    {clusters.find((c) => c.id === g.clusterId)?.description || '설명 없음'}
                  </span>
                </span>
              </span>
