import { Server, CheckCircle, AlertTriangle, XCircle, WifiOff } from 'lucide-react';
...
export function ClusterSidebar({
  clusters, selectedId, onSelect, title = '클러스터', highlightActive = true,
}: ClusterSidebarProps) {
  return (
    <aside className="w-52 flex-shrink-0 bg-card border border-border rounded-xl p-2 h-fit sticky top-4">
      <p className="px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
        {title}
        <span className="ml-1 text-muted-foreground/60">({clusters.length})</span>
      </p>
